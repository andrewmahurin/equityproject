...
for (var in list(dailychange, dailyhigh,dailylow)) {
  ...
  model=lm((hightolow) ~ #for use in "selling high"
              + lag2(monthlychange)
              +  lag2(dailyvol) + lag3(dailyvol) + lag4(dailyvol)+ lag1(log(high))
model=lm((hightolow) ~ #for use in "selling high"
    +  lag2(dailyvol) + lag3(dailyvol) + lag4(dailyvol)+ lag1(log(high))
    + lag1(dailychange)        
    + lag2(dailychange)
...
#     +lag1(dailylow > -.005) # conditional
           
           , data= x);print(summary(model))
+ lag2(monthlychange)
#, lag1(uptoclose> .001)
  fitted(model)
  plot(offset(model), (hightolow))
  lag1(hightohigh) + 
  lag1(hightohigh) + 
  , lowtohigh,dailyhigh, dailylow
  ...
  lag1(dailylow)+ 
    + lag2(weeklychange) 
  model=lm(cbind(lowtohigh) ~ 
             #lag1(hightohigh) + lag1(lowtolow) + 
             lag1(log(close)) + lag1(dailychange) + lag1(log(volume))
            + lag1(weeklychange) + lag1(monthlychange)
            , data= x);print(summary(model))
  #lag1(hightohigh) + lag1(lowtolow) + 
  #           lag1(log(close)) + 
  #         + lag1(log(volume)) 
  #+ lag1(weeklychange) + lag1(monthlychange)
  #lag1(hightolow) + lag1(lowtohigh) +
...
stats(monthlychange)
  stats =function (x){
  print(a <-mean(x,na.rm = TRUE))
  print(b <- sd(x,na.rm = TRUE))
  print(a/b)
  }
